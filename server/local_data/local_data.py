...
columns = [
    'Azteca Avg', 'Competition Avg',
    'Azteca Note Avg', 'Competition Note Avg',
    'Azteca Video Avg', 'Competition Video Avg', 'Date'
]
...
def init(inner_data=None):
    ...
    df['Azteca Avg'] = df[azteca_columns].mean(axis=1).round(1)
    df['Competition Avg'] = df[competition_columns].mean(axis=1).round(1)

    df['Azteca Note Avg'] = df[[
    ...
    df['Azteca Avg Change'] = pct_change(df['Azteca Avg'])
    df['Competition Avg Change'] = pct_change(df['Competition Avg'])

    df['Azteca Note Change'] = pct_change(df['Azteca Note Avg'])
    df['Competition Note Change'] = pct_change(df['Competition Note Avg'])

    df['Azteca Video Change'] = pct_change(df['Azteca Video Avg'])
...
def calculate_relevant_insights(filtered_df, companies, title, date_filter=None):
    ...
    if significant_changes:
        most_relevant = max(significant_changes, key=lambda x: abs(x[3]))
        insight = f"Azteca {most_relevant[1]} {most_relevant[2]} by {abs(most_relevant[3]):.1f}%, especially in {most_relevant[0]}."
    else:
