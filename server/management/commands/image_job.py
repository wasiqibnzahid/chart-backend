...
def process_sitemap(site, semaphore, date):
    with semaphore:
        print(f"Processing sitemap for {site.name}: {site.sitemap_url} {"image-sitemap-latest.xml" not in site.sitemap_url} {"sitemap-image.xml" in site.sitemap_url}")
        if "image-sitemap-latest.xml" not in site.sitemap_url and "sitemap-image.xml" in site.sitemap_url:
            site.sitemap_url = site.sitemap_url.replace("sitemap-image.xml", "image-sitemap-latest.xml");
            site.save();
        try:
