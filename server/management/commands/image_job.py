...
def process_sitemap(site, semaphore):
    with semaphore:
        ...
        try:
            ...
            for url in root.findall('.//ns:url', namespaces):
                ...
                if len(images) > 1:  # Only process if there's more than one image
                    ...
                    try:
                        ...
                        res = get_lighthouse_mobile_score(page_url, "IMAGE JOB", log_file_name=OTHER_RECORD_FILEPATH)
                        ...
                        if res["performance_score"] != 0:
                            ...
                            print(f"Processed URL {page_url} with {len(images)} images")
            ...
            print(f"Created record for {site.name} with performance score {metrics['performance_score']}")
            return record
                    
        except Exception as e:
            print(f"Error processing sitemap for {site.name}: {str(e)}")
            write_text_to_file(f"Error processing sitemap for {site.name}: {str(e)}", OTHER_RECORD_FILEPATH)
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_site):
                ...
                try:
                    ...
                    write_text_to_file(f"Error processing site {site.name}: {str(e)}", OTHER_RECORD_FILEPATH) 